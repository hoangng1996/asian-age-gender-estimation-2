...
def main():
    ...
    root_path = "megaage_asian/megaage_asian/train/"
    logging.debug("Loading data...")
    #image, gender, age, _, image_size, _ = load_data(input_path)
    image_size = 64
    weight_file = os.path.join("pretrained_models", "weights.18-4.06.hdf5")

    out_imgs = np.empty((len(os.listdir(root_path))-30000, image_size, image_size, 3 ))
    i = 0
    for filename in natsorted(os.listdir(root_path)):
        img = cv2.imread(root_path + filename)
        out_imgs[i] = (cv2.resize(img, (image_size, image_size)))
        i += 1
        if i==10000:
            break
    X_data = out_imgs
    ...
    y_data_g = []
    j = 0
    with open('megaage_asian/megaage_asian/list/train_age2.txt') as f:
        for line in f:
            y_data_g.append(int(line.split()[1]))
            y_data_a.append(int(line.split()[0]))
            j += 1
            if j == 10000:
                break
        f.close()
    ...
    y_data_g = np_utils.to_categorical(y_data_g, 2)
    y_data_a = np_utils.to_categorical(y_data_a, 101)
    print(X_data.shape)
